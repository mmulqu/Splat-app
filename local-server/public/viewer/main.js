...
void main () {
    uvec4 cen = texelFetch(u_texture, ivec2((uint(index) & 0x3ffu) * 2u, uint(index) >> 10), 0);
    ...
    uvec4 cov = texelFetch(u_texture, ivec2(((uint(index) & 0x3ffu) * 2u) | 1u, uint(index) >> 10), 0);
    ...
    float lambda1 = mid + sqrt(max(0.1, mid * mid - (cov2d[0][0] * cov2d[1][1] - cov2d[0][1] * cov2d[1][0])));
    float lambda2 = mid - sqrt(max(0.1, mid * mid - (cov2d[0][0] * cov2d[1][1] - cov2d[0][1] * cov2d[1][0])));
    float radius = ceil(3.0 * sqrt(max(lambda1, lambda2)));
    vec2 pointCenter = pos2d.xy / pos2d.w;

    vColor = vec4(cen.w & 0xffu, (cen.w >> 8) & 0xffu, (cen.w >> 16) & 0xffu, (cen.w >> 24) & 0xffu) / 255.0;
    vPosition = position;

    gl_Position = vec4(pointCenter + position * radius * 2.0 / viewport, 0.0, 1.0);
}
`;
...
function main() {
    ...
    const url = params.get("url") || "train.splat";
    ...
    req.onerror = (e) => {
        console.error("Failed to load PLY file:", e);
        document.body.innerHTML = `<div style="color: white; padding: 20px;">Error loading file: ${url}</div>`;
    };
    ...
    req.onload = (e) => {
        console.log("File loaded, size:", req.response.byteLength, "bytes");
        const buffer = req.response;
        if (url.endsWith(".ply")) {
            console.log("Processing PLY file...");
            worker.postMessage({ ply: buffer });
        } else {
            console.log("Not a PLY file, downloading...");
    ...
    var u_buffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, u_buffer);
    ...
    window.addEventListener("keyup", (e) => {
        ...
        function render() {
            ...
            const cameraPosition = [x + target[0], y + target[1], z + target[2]];
            ...
            const lenS = Math.hypot(s[0], s[1], s[2]);
            s[0] /= lenS;
            s[1] /= lenS;
            s[2] /= lenS;
            const u = [
            ...
            const view = [
                ...
                -f[2],
                0,
                -s[0] * cameraPosition[0] -
                s[1] * cameraPosition[1] -
                s[2] * cameraPosition[2],
                -u[0] * cameraPosition[0] -
                u[1] * cameraPosition[1] -
                u[2] * cameraPosition[2],
                f[0] * cameraPosition[0] +
                f[1] * cameraPosition[1] +
                f[2] * cameraPosition[2],
                1,
            ];
            ...
            const viewProj = multiply4(projectionMatrix, view);
            ...
            gl.uniform2fv(u_focal, [1159.58, 1164.66]);
            gl.uniform2fv(u_viewport, [width, height]);
